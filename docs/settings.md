...
| `xray.enableXrayPanel` | boolean | `true` | Show the Xray Traceability panel beneath the Steps panel. Disabling hides the view and tears down its tree, model, and file watchers. See [features.md → Xray Traceability panel](features.md#xray-traceability-panel). |
| `xray.siteUrl` | string | `` | Your Jira/Xray Cloud site host, e.g. `acme.atlassian.net`. Used to build `https://{siteUrl}/browse/{KEY}` links for the **Open Xray Issue in Browser** action. A pasted full URL or trailing slash is tolerated — the value is normalized to a bare host. Empty: the open action shows a warning instead of guessing. |
| `xray.testTagPrefix` | string | `TEST_` | Tag prefix that maps a scenario to an Xray test — `@TEST_CALC-1043` links to test `CALC-1043`. The prefix is matched case-insensitively; an empty or whitespace value falls back to the default. |
| `xray.reqTagPrefix` | string | `REQ_` | Tag prefix that marks a scenario as covering an Xray requirement — `@REQ_CALC-900` covers requirement `CALC-900`. Same case-insensitivity and empty-value fallback as `xray.testTagPrefix`. |
