...
| `traceability.enablePanel` | boolean | `true` | Show the Traceability panel beneath the Steps panel. Disabling hides the view and tears down its tree, model, and file watchers. See [features.md → Xray Traceability panel](features.md#xray-traceability-panel). |
| `traceability.provider` | string (`xray`) | `xray` | Which ALM backend the Traceability panel maps to. Only `xray` (Jira Xray Cloud) is available today; more providers land in later releases. |
| `traceability.testTagPrefix` | string | `TEST_` | Tag prefix that maps a scenario to a test in the tracker — `@TEST_CALC-1043` links to test `CALC-1043`. The prefix is matched case-insensitively; an empty or whitespace value falls back to the default. |
| `traceability.reqTagPrefix` | string | `REQ_` | Tag prefix that marks a scenario as covering a requirement — `@REQ_CALC-900` covers requirement `CALC-900`. Same case-insensitivity and empty-value fallback as `traceability.testTagPrefix`. |
| `xray.siteUrl` | string | `` | Your Jira/Xray Cloud site host, e.g. `acme.atlassian.net`. Used to build `https://{siteUrl}/browse/{KEY}` links for the **Open Issue in Tracker** action. A pasted full URL or trailing slash is tolerated — the value is normalized to a bare host. Empty: the open action shows a warning instead of guessing. |
