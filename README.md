...
- VS Code declares `1.99.0` as its minimum host. CI tests that exact host and one pinned current target.
...
The [compatibility record](docs/compatibility.md) names the exact VS Code, Playwright, and legacy `playwright-bdd` versions represented by current evidence. Playwright JSON is a legacy compatibility boundary, not a provider protocol.
